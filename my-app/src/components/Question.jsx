...
function Question() {
    const items = ['q101', 'q102', 'q103', 'q104', 'q105', 'q106', 'q107', 'q108', 'q109', 'q110'];
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:3001/data/${randomItem}`)
            .then((response) => {
    ...
    useEffect(() => {
        if (user) {
            axios.get(`http://localhost:3001/scoreget1/${user}`)
                .then((res) => {
                    const data = res.data[0].gate1;
                    console.log("gateno", data);
                    setscore1(data);
                })
                .catch((err) => {
                    console.error(err);
    ...
    const checkanswer = (e) => {
        ...
        if (userAnswer === answer) {
            ...
            console.log(fscore);
            axios.put(`http://localhost:3001/edit1/${user}/`, { gate1: fscore })
                .then((response) => {
